...
function computeVariance(plannedByGroup, actualByGroup, thresholds) {
  ...
  for (const key of Object.keys(actualByGroup)) {
...
function buildSection(title, variance, totalBudget, totalActual, currency) {
  ...
  for (const [level, label] of groups) {
    ...
    for (const item of items) {
      lines.push(`- ${item.group}: ${formatMoney(item.actual, currency)} / ${formatMoney(item.planned, currency)} (${formatPercent(item.usagePercent)})`);
    }
    lines.push('');
  }
...
function buildPayload(input) {
  ...
  const thresholds = {
    watch: input.watchThresholdPercent ?? 80,
    warning: input.alertThresholdPercent ?? 100,
    critical: input.criticalThresholdPercent ?? 120
  };
  ...
  const coreTotalBudget = sum(coreBudget);
  const coreTotalActual = sum(coreActual);
  const aelfTotalBudget = sum(aelfBudget);
  const aelfTotalActual = sum(aelfActual);
  const totalPlanned = round2(coreTotalBudget + aelfTotalBudget);
  const totalActual = round2(coreTotalActual + aelfTotalActual);
  ...
  const scoped = [
    ['core', coreVariance],
    ['aelf', aelfVariance],
    ['coreByBusinessUnit', coreByBusinessUnit]
  ];
  const highlights = scoped.flatMap(([scope, items]) => items
    .filter((item) => ['critical', 'warning', 'watch'].includes(item.level))
    .map((item) => ({ ...item, scope, reason: item.usagePercent === -1 ? 'Actual spend exists without planned budget' : `${item.usagePercent}% usage` })));
  ...
  const card = {
    ...
    header: { title: { tag: 'plain_text', content: `Budget variance summary ${period}` }, template },
    elements: [
      { tag: 'div', text: { tag: 'lark_md', content: `Data cutoff: ${dataCutoff || 'N/A'}` } },
      { tag: 'hr' },
      { tag: 'div', text: { tag: 'lark_md', content: buildSection('Core budget', coreVariance, coreTotalBudget, coreTotalActual, currency) } },
      { tag: 'hr' },
      { tag: 'div', text: { tag: 'lark_md', content: buildSection('aelf budget', aelfVariance, aelfTotalBudget, aelfTotalActual, currency) } },
      { tag: 'hr' },
      { tag: 'div', text: { tag: 'lark_md', content: buildSection('Core by business unit', coreByBusinessUnit, coreTotalBudget, coreTotalActual, currency) } }
    ]
  };

  const lark = { msg_type: 'interactive', card };
  if (input.receiveId) {
    lark.body = { receive_id: input.receiveId, msg_type: 'interactive', content: JSON.stringify(card) };
  }
  ...
  return {
    ...
    currency,
    dataCutoff,
    thresholds: { watchPercent: thresholds.watch, warningPercent: thresholds.warning, criticalPercent: thresholds.critical },
    totals: { planned: totalPlanned, actual: totalActual, variance: totalVariance, usagePercent: totalUsagePercent, variancePercent: totalVariancePercent },
    sections: {
      core: { totalBudget: coreTotalBudget, totalActual: coreTotalActual, variance: coreVariance },
      aelf: { totalBudget: aelfTotalBudget, totalActual: aelfTotalActual, variance: aelfVariance },
      coreByBusinessUnit
    },
    highlights,
    lark
